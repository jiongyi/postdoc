...
def measure_comet_props(npf_im, npf_bw_im, pulse_im, chase_im, actin_bw_im):
    ...
    for i in range(1, no_beads + 1):
        ...
        i_tail_axis_bw_im = skeletonize(i_tail_bw_im)
...
def batch_analysis(folder_path_str):
    ...
    for i in range(no_files):
        ...
        i_sum_im = i_npf_im / max(i_npf_im.flatten()) + i_pulse_im / max(i_pulse_im.flatten()) + i_chase_im / max(i_chase_im.flatten())
        i_actin_bw_im = binarize_actin(i_sum_im)

...
from skimage.morphology import closing, disk, erosion, dilation, opening, reconstruction, remove_small_objects, skeletonize
from skimage.filters import threshold_triangle, threshold_isodata, threshold_otsu, gaussian
from skimage.exposure import equalize_adapthist
from numpy import array, mean, nan, stack, sum, zeros, linspace, pi, tan, inf, sqrt, unique, max
...
def estimate_comet_tail_props(file_path_str):
    ...
    if no_beads > 0:
        c1_bw_im = gaussian(c1_im, sigma = 1) > max([threshold_otsu(c1_im), c1_im[~comet_bw_im].mean() + 3 * c1_im[~comet_bw_im].std()])
        c2_bw_im = gaussian(c2_im, sigma = 1) > max([threshold_otsu(c2_im), c2_im[~comet_bw_im].mean() + 3 * c2_im[~comet_bw_im].std()])
        ...
        for i in range(no_beads):
            ...
            comet_tail_axial_ratio_row[i] = i_comet_regionprops_list[0].major_axis_length / i_comet_regionprops_list[0].minor_axis_length

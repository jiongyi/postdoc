...
class network(object):
    def __init__(self, no_filaments = 200, elong_rate = 45, branch_rate = 0.01, cap_rate = 0.5):
    ...
    def update_ratchet_weight(self):
        ...
        if no_proximal_free == 0:
            ...
            average_force = 1.0 * self.MONOMER_WIDTH * 0.4 / 4.114 * no_proximal / no_proximal_free
            self.ratchet_weight = exp(-average_force * self.MONOMER_WIDTH / 4.114)
        ...
        for i in range(self.no_filaments):
            if self.is_capped_row[i]:
                ...
                if rand() <= self.branch_rate:
                    self.branch(i)
                    continue
                elif rand() <= (self.ratchet_weight * self.elong_rate * self.TIME_INTERVAL):
                    self.elongate(i)
                    continue
                elif rand() <= (self.ratchet_weight * self.cap_rate * self.TIME_INTERVAL):
                    self.cap(i)
                    continue

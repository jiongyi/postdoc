...
init_ppr_eq_row, init_wh2_eq_row, init_ends_eq_row = steady_state(nucleation_const = init_nucleation_const)
data_source = ColumnDataSource(data = dict(x = init_capping_rate_row, Nucleation = init_nucleation_const * init_wh2_eq_row**2 * init_ends_eq_row, E = init_ends_eq_row))
...
plot_hand.line('x', 'Nucleation', source = data_source, line_width = 3, line_color = 'Red', legend = 'Nucleation rate')
plot_hand.line('x', 'E', source = data_source, line_width = 3, line_color = 'Blue', legend = 'Barbed-end density')
...
def update_steady_state(attrname, old, new):
    ...
    data_source.data = dict(x = new_capping_rate_row, Nucleation = new_nucleate_const * new_wh2_eq_row**2 * new_ends_eq_row, E = new_ends_eq_row)

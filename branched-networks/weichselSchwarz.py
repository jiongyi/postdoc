...
class network(object):
    ...
    def timeStep(n, dt, Fext = 0.0):
        ...
        for x, dx in n.Frontier:
            ...
            if polRate > 0.0:
                n.elongate(i)
            # Cap.
            capRate = n.rKappa(x[0], x[1] - yLastFront, n.tElapsed)
            if capRate > 0.0:
                if exponential(scale = 1.0 / capRate, size = 1) <= dt:
                    n.cap(i)
                    continue
            # Branch.
            branchRate = n.rBeta(x[0], x[1] - yLastFront, n.tElapsed) / noActive
            if branchRate > 0.0:
        ...
        if n.recordHistory == True:
            ...
            n.active = append(n.active, [noActive])
    ...
    def plotData(n):
        ...
        contourf(xGrid, yGrid, zGrid, cmap = 'Greens', vmin = 1000.0, vmax = 1001)
    ...
    def plotAngles(n):
        ...
        angleBins = linspace(-91, 91, 18) * pi / 180
        hist, bins = histogram(n.getAngles(n.Frontier), bins = angleBins, normed = False)
        centers = (bins[1:] + bins[:-1]) / 2.0
        
        # Plot polar histogram.
        ax = subplot(111, projection = "polar")
        ax.bar(centers, hist, color =  'g', edgecolor = "none", align = "center")
